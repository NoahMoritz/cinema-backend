...
/**
 * Filter that can be configured to be used in a web.xml file. Needs the init parameter 'applicationClass' set to the
 * application class where the adding of routes should be made.
 *
 * @author Per Wendel
 */
public class SparkFilter implements Filter {
    ...
    /**
     * It contains all the Spark application instances that was declared in the filter configuration. They can be one or
     * more class names separated by commas.
     */
    ...
    /**
     * Returns an instance of {@link SparkApplication} which on which {@link SparkApplication#init() init()} will be
     * called. Default implementation looks up the class name in the filterConfig using the key {@value
     * #APPLICATION_CLASS_PARAM}. Subclasses can override this method to use different techniques to obtain an instance
    ...
    /**
     * Returns the instances of {@link SparkApplication} which on which {@link SparkApplication#init() init()} will be
     * called. Default implementation looks up the class names in the filterConfig using the key {@value
     * #APPLICATION_CLASS_PARAM}. Subclasses can override this method to use different techniques to obtain an instance
     * (i.e. dependency injection).
     *
     * @param filterConfig the filter configuration for retrieving parameters passed to this filter.
