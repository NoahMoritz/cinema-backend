...
/**
 * Holds the routes and performs matching from HTTP requests to routes. Works as Sinatra's, ie. if there are more than
 * one match the one that was mapped first is chosen.
 *
 * @author Per Wendel
 */
public class Routes {
    ...
    /**
     * Removes a particular route from the collection of those that have been previously routed. Search for a previously
     * established routes using the given path and HTTP method, removing any matches that are found.
     *
     * @param path       the route path
     * @param httpMethod the http method
     * @return <tt>true</tt> if this a matching route has been previously routed
     * @throws IllegalArgumentException if <tt>path</tt> is null or blank or if <tt>httpMethod</tt> is null, blank or an
     *                                  invalid HTTP method
     * @since 2.2
     */
    ...
    /**
     * Removes a particular route from the collection of those that have been previously routed. Search for a previously
     * established routes using the given path and removes any matches that are found.
     *
     * @param path the route path
     * @return <tt>true</tt> if this a matching route has been previously routed
