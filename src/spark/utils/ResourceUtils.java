 ...
 * for handling all kinds of file resources in a uniform manner. org.springframework.core.io.ResourceLoader @code
 * getResource() method can resolve any location to a org.springframework.core.io.Resource object, which in turn allows
 * one to obtain a {@code java.io.File} in the file system through its {@code getFile()} method.
 * <p>The main reason for these utility methods for resource location handling
 * is to support Log4jConfigurer, which must be able to resolve resource locations <i>before the logging system has been
 * initialized</i>. Spring's {@code Resource} abstraction in the core package, on the other hand, already expects the
