...
/**
 * Represents a Spark server "session". If a user wants multiple 'Sparks' in his application the method {@link
 * Service#ignite()} should be statically imported and used to create instances. The instance should typically be named
 * so when prefixing the 'routing' methods the semantic makes sense. For example 'http' is a good variable name since
 * when adding routes it would be: Service http = ignite(); ... http.get("/hello", (q, a) {@literal ->} "Hello World");
 */
public final class Service extends Routable {
    ...
    /**
     * Sets the external folder serving static files. <b>Observe: this method must be called before all other
     * methods.</b>
     *
    ...
    /**
     * Immediately stops a request within a filter or route NOTE: When using this don't catch exceptions of type
     * HaltException, or if catched, re-throw otherwise halt will not work
     *
    ...
    /**
     * Immediately stops a request within a filter or route with specified status code NOTE: When using this don't catch
     * exceptions of type HaltException, or if catched, re-throw otherwise halt will not work
     *
    ...
    /**
     * Immediately stops a request within a filter or route with specified body content NOTE: When using this don't
    ...
    /**
     * Immediately stops a request within a filter or route with specified status code and body content NOTE: When using
     * this don't catch exceptions of type HaltException, or if catched, re-throw otherwise halt will not work
     *
     * @param status The status code
     * @param body   The body content
    ...
    /**
     * Set the IP address that Spark should listen on. If not called the default address is '0.0.0.0'. This has to be
     * called before any route mapping is done.
     *
    ...
    /**
     * Add a path-prefix to the routes declared in the routeGroup The path() method adds a path-fragment to a
     * path-stack, adds routes from the routeGroup, then pops the path-fragment again. It's used for separating routes
     * into groups, for example: path("/api/email", () -> { ....post("/add",       EmailApi::addEmail);
    ...
    /**
     * Set the port that Spark should listen on. If not called the default port is 4567. This has to be called before
     * any route mapping is done. If provided port = 0 then the an arbitrary available port will be used.
     *
    ...
    /**
     * Set the connection to be secure, using the specified keystore and truststore. This has to be called before any
     * route mapping is done. You have to supply a keystore file, truststore file is optional (keystore will be reused).
     * By default, client certificates are not checked. This method is only relevant when using embedded Jetty servers.
     * It should not be used if you are using Servlets, where you will need to secure the connection in the servlet
     * container
     *
     ...
     * Set the connection to be secure, using the specified keystore and truststore. This has to be called before any
     * route mapping is done. You have to supply a keystore file, truststore file is optional (keystore will be reused).
     * This method is only relevant when using embedded Jetty servers. It should not be used if you are using Servlets,
     * where you will need to secure the connection in the servlet container
     *
     * @param keystoreFile       The keystore file location as string
     * @param keystorePassword   the password for the keystore
     * @param truststoreFile     the truststore file location as string, leave null to reuse keystore
     * @param needsClientCert    Whether to require client certificate to be supplied in request
     * @param truststorePassword the trust store password
     * @return the object with connection set to be secure
     */
    public synchronized Service secure(String keystoreFile,
    ...
    public final class StaticFiles {
        ...
        /**
         * Puts custom header for static resources. If the headers previously contained a mapping for the key, the old
        ...
        /**
         * Puts custom headers for static resources. If the headers previously contained mapping for a specific key in
         * the provided headers map, the old value is replaced by the specified value.
         *
        ...
        /**
         * Sets the folder in classpath serving static files. Observe: this method must be called before all other
         * methods.
         *
         * @param folder the folder in classpath.
         */
